...
|---------|-----|------|---------|-----------------|
| **multimodal-embedding** | `services/multimodal-embedding/` | 8001 | Text/image embedding via Qwen3-VL-Embedding-2B (CPU PyTorch) or CLIP (ONNX CPU). Outputs 2048-dim vectors. | `cd services/multimodal-embedding && uvicorn src.app:app --host 0.0.0.0 --port 8001` |
| **document-ingestion** | `services/document-ingestion/` | 8000 | Ingest PDF/DOCX/TXT/URL → chunk → embed → store in LanceDB. Also exposes ZIM stub. | `cd services/document-ingestion && uvicorn app.main:app --host 0.0.0.0 --port 8000` |
| **reranker** | `services/reranker/` | 8003 | Cross-encoder reranking via Qwen3-VL-Reranker-2B (CPU PyTorch). Scores (query, text) pairs. | `cd services/reranker && uvicorn app.main:app --host 0.0.0.0 --port 8003` |
...
| **vlm-proxy** | `services/vlm-proxy/` | 8082 | Thin proxy: extracts video frames → forwards to llama-server. **Skip if no video input needed.** | `cd services/vlm-proxy && uvicorn app.proxy:app --host 0.0.0.0 --port 8082` |
| **chat-qa** | `services/chat-qa/` | 8080 | RAG chatbot: LanceDB retrieval → rerank → LLM via streaming SSE. | `cd services/chat-qa && uvicorn app.server:app --host 0.0.0.0 --port 8080` |
| **audio-analyzer** | `services/audio-analyzer/` | 8002 | Audio/video transcription via pywhispercpp (whisper.cpp, CPU). | `cd services/audio-analyzer && uvicorn audio_analyzer.main:app --host 0.0.0.0 --port 8002` |
| **model-download** | `services/model-download/` | 8084 | REST API for downloading HuggingFace / Ollama models. OpenVINO plugin disabled on ARM64; HF and Ollama plugins work natively. | `cd services/model-download && ENABLED_PLUGINS=huggingface,ollama uvicorn src.main:app --host 0.0.0.0 --port 8084` |
| **multilevel-video-understanding** | `services/multilevel-video-understanding/` | 8085 | Video summarisation using multi-level VLM prompting. Requires an OpenAI-compatible LLM endpoint. | `cd services/multilevel-video-understanding && VLM_ENDPOINT_URL=http://localhost:8082/v1 uvicorn video_analyzer.main:app --host 0.0.0.0 --port 8085` |
...
> with `pip install -e services/video-chunking-utils/` and import
...
`services/shared/` and running `pip install -e services/shared/`.
